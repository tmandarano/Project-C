...
require_once('localhost/models/tag.php');
...
class PhotoDAO
{
    ...
    public function save($photo)
    {
        ...
        $sql = "INSERT INTO photo (url, location, date_added, date_modified) VALUES (:url, :location, NOW(), NOW())";
    ...
    public function update($photo)
    {
        ...
        $sql = "UPDATE photo set url = :url, date_modified = NOW() where id = :id";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(":url", $photo->getUrl(), PDO::PARAM_STR);
    ...
    private static function recordsToPhotos($records) 
    {
        ...
        foreach($records as $record) {
            ...
            $photo->setLatitude($record['latitude']);
            $photo->setLongitude($record['longitude']);
            $photo->setLocation($record['location']);
            $photo->setCaption($record['caption']);
            $photo->setDateAdded($record['date_added']);
            $photo->setDateModified($record['date_modified']);
